...
def initialize_minimal_data():
    """Initialize only essential data for quick startup"""
    ...
    geofence_data.extend([
        {
            ...
            'trigger_count': 150, 'energy_savings': 250,
        ...
        },
        {
            ...
            'trigger_count': 100, 'energy_savings': 175,
...
def initialize_models_async():
    ...
    global energy_model, ridge_model, anomaly_detector, scaler, mlp_model, models_initialized
...
def train_models_minimal():
    """Train models with minimal data for faster initialization"""
    ...
    try:
        df = pd.DataFrame(energy_data[-50:]) 
...
def train_models():
    """Full model training - called when more data is available"""
    global energy_model, ridge_model, anomaly_detector, scaler, mlp_model
    
    if not models_initialized or len(energy_data) < 50:
        return
    ...
    try:
        ...
        energy_model.n_estimators = 100
        energy_model.max_depth = 10
        ...
        print(f"Full model training error: {e}")

@lru_cache(maxsize=100)
def detect_dynamic_anomalies_cached(data_hash):
    """Cached version of anomaly detection"""
    return detect_dynamic_anomalies(pd.DataFrame(energy_data))
...
def detect_dynamic_anomalies(df):
    ...
    if len(recent_data) > 30 and models_initialized:
        try:
...
def update_device_states():
    ...
    if len(energy_data) % 50 == 0 and models_initialized:
...
def get_energy_data():
    ...
    if not models_initialized:
        # Return data without predictions if models aren't ready
        return jsonify(recent_data)
    
    for item in recent_data:
        try:
            features = np.array([[item['hour'], item['day_of_week'], item['temperature'],
...
@app.route('/api/total-savings', methods=['GET'])
def get_total_savings():
    total_savings = sum(float(g.get('energy_savings', 0)) for g in geofence_data)
