...
def get_energy_data():
    ...
    for item in recent_data:
        try:
            ...
            rf_pred = energy_model.predict(features)
            mlp_pred = mlp_model.predict(scaler.transform(features))
            try:
...
def warmup_task():
    ...
    try:
        if len(energy_data) > 0:
            sample = energy_data[-1]
            features = np.array([[sample['hour'], sample['day_of_week'], sample['temperature'], 
                sample['occupancy'], sample.get('device_consumption', 0),
                sample.get('time_factor', 1.0), sample.get('weather_factor', 1.0),
                np.sin(2 * np.pi * sample['hour'] / 24),
                np.cos(2 * np.pi * sample['hour'] / 24),
                np.sin(2 * np.pi * sample['day_of_week'] / 7),
                np.cos(2 * np.pi * sample['day_of_week'] / 7)
            ]])
            _ = energy_model.predict(features)
    except Exception:
        pass
